...
describe('SignUp Controller', () => {
  ...
  test('Should return 400 if no connfirmation password is provided', async () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'any_name',
  ...
  test('Should return 500 if EmailValidator throws', async () => {
    ...
    const httpResponse = await sut.handle(makeFakeRequest())
    expect(httpResponse).toEqual(serverError(new ServerError('any_server_error_stack_message')))
  })
